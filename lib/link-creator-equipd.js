...
export default {
	...
	parse() {
		...
		if (editor = atom.workspace.getActiveTextEditor()) {
			...
			while ((m = regex.exec(selection)) !== null) {
					...
					finalText = finalText.replace(scriptureBefore, urlToEquipD);
			}
		}
	...
	processMatch(search) {
		...
		if (search[1] !== undefined) {
			if (search[1] == '[') {
				return '[' + libro.trim() + ' ' + chapter.trim() + ':' + verses.trim();
			} else if (search[1] == '='){
				return '=' + abreviation + chapter + ':' + verses.replace(' ', '');
			} else {
				return this.replacer('[{0} {1}:{2}](equipdbible://x-callback-url/lookup?x-source=editorial&language=es&scripture={3}{4}:{5}&x-success=editorial://)', libro.trim(), chapter.trim(), verses, abreviation.trim(), chapter.trim(),verses.trim().replace(' ', ''));
			}
		} else {
			return this.replacer('[{0} {1}:{2}](equipdbible://x-callback-url/lookup?x-source=editorial&language=es&scripture={3}{4}:{5}&x-success=editorial://)', libro.trim(), chapter.trim(), verses, abreviation.trim(), chapter.trim(),verses.trim().replace(' ', ''));
		}
